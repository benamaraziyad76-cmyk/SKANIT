...
async function crop(srcFile, destFile, region) {
  try {
    if (!fs.existsSync(srcFile)) {
      console.log(`  ⚠️ Fichier introuvable: ${path.basename(srcFile)}`);
      return;
    }
    ...
    // On prend une zone large pour s'assurer que le plat est entier
    ...
    // Extraction et centrage parfait sur un carré blanc
    await sharp(srcFile)
      .extract({ left, top, width, height })
      .flatten({ background: '#ffffff' })
      .resize(600, 600, { 
         fit: 'contain', // contain permet de garder toute l'image dans le cadre sans couper
...
async function main() {
  console.log('\n=== DECOUPE EXACTE DES NOUVELLES PLANCHES ===\n');

  // 1. SANDWICHES (3639.jpg)
  const sw = path.join(SRC_DIR, '3639.jpg');
  console.log('🥪 Découpe des Sandwiches...');
  ...
  // Les autres Naans (texte) utiliseront les photos existantes ou par defaut
  await crop(naan, 'naan-tandori.jpg',     { left: 0.53, top: 0.35, width: 0.42, height: 0.18 }); // Recyclage Tenders
  await crop(naan, 'naan-poulet-curry.jpg',{ left: 0.53, top: 0.35, width: 0.42, height: 0.18 }); // Recyclage Tenders
  await crop(naan, 'naan-chicken.jpg',     { left: 0.53, top: 0.15, width: 0.42, height: 0.18 }); // Recyclage Royal
  await crop(naan, 'naan-veggie.jpg',      { left: 0.05, top: 0.55, width: 0.42, height: 0.18 }); // Recyclage Steak
  await crop(naan, 'naan-special.jpg',     { left: 0.05, top: 0.15, width: 0.42, height: 0.18 }); // Recyclage Radical
  await crop(naan, 'naan-montagnard.jpg',  { left: 0.05, top: 0.35, width: 0.42, height: 0.18 }); // Recyclage Boursin
  await crop(naan, 'naan-phenomene.jpg',   { left: 0.05, top: 0.55, width: 0.42, height: 0.18 }); // Recyclage Steak

  // 4. NAAN BURGER (3656.jpg)
  const naanburger = path.join(SRC_DIR, '3656.jpg');
  console.log('\n🍔 Découpe des Naan Burgers...');
  ...
  console.log('\n✅ C\'est terminé ! Les images sont découpées et parfaitement centrées.');
}
